...
const Layers = () => {
  ...
  useEffect(() => {
    ...
    layer_names.forEach(function (lyr_name) {
      var newLayer = (geojson_layers[lyr_name] = L.geoJson(null, {
        style: function (feature) {
          return geojson_styles[lyr_name];
        },
      }).addTo(map));
      control.addOverlay(newLayer, lyr_name);
    });
    ...
    const sdat_rest_query_base =
      "https://geodata.md.gov/imap/rest/services/PlanningCadastre/MD_ParcelBoundaries/MapServer/0/query";
    const geoserver_soil_query_base =
      "https://bitbybitmapping.com/geoserver/numan/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=numan%3Amapunits_with_attributes&maxFeatures=50&srsName=EPSG:4326&outputFormat=application/json&";
    const geoserver_watersheds_query_base =
      "https://bitbybitmapping.com/geoserver/numan/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=numan%3Awatersheds&maxFeatures=50&srsName=EPSG:4326&outputFormat=application/json&";
    const geoserver_counties_query_base =
      "https://bitbybitmapping.com/geoserver/numan/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=numan%3Acounties&maxFeatures=50&srsName=EPSG:4326&outputFormat=application/json&";
    const geoserver_physiographic_query_base =
      "https://bitbybitmapping.com/geoserver/numan/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=numan%3Aphysiographic&srsName=EPSG:4326&maxFeatures=50&outputFormat=application/json&";
    ...
    function evaluateAllFields() {
      ...
      fields.forEach(function (field) {
        ...
        field
          .bindTooltip("Field " + this_index.toString(), { permanent: true })
          .openTooltip();
        ...
        let esri_object_rings = JSON.stringify(
          esri_object["geometry"]["rings"]
        );
        let geometry_filter =
          '{"rings":' +
          esri_object_rings +
        ...
        let coords_projected_mercator = field._latlngs[0].map((x) =>
          proj4("EPSG:4326", "EPSG:3857", [x.lng, x.lat])
        );
        coords_projected_mercator.push(coords_projected_mercator[0]);
        let formatted_coords = coords_projected_mercator
          .map((x) => x[0].toString() + " " + x[1].toString())
          .toString();
        //field_polys.push("((" + formatted_coords + "))");
        let cql_filter =
          "cql_filter=INTERSECTS(geom,MULTIPOLYGON(((" +
          formatted_coords +
          "))))";
        ...
        // // Watershed query
        $.post(
          geoserver_watersheds_query_base,
          cql_filter,
          function (watershed_data) {
            geojson_callback(
              watershed_data,
              field_geojson,
              query_set,
        ...
        $.post(
          geoserver_physiographic_query_base,
          cql_filter,
          function (physiographic_data) {
            geojson_callback(
              physiographic_data,
              field_geojson,
              query_set,
    ...
    $("#fileInput").change(function (e) {
      ...
      if (lower_fn.endsWith(".kmz")) {
        let zip = new JSZip();
        zip
          .loadAsync(this.files[0] /* = file blob */)
          .then(function (zip) {
